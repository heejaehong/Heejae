...
<div class="tg-mobile visible-xs visible-sm visible-md">
				...
				<?php } ?>
					...
					<?php if($theme_options->get( 'category_page_display_add_to_cart' ) != '0') { ?>
						<div class="add-to-cart hidden-xs hidden-sm hidden-md"><a onclick="cart.add('<?php echo $product['product_id']; ?>');" class="button"><span><?php if($theme_options->get( 'add_to_cart_text', $config->get( 'config_language_id' ) ) != '') { echo $theme_options->get( 'add_to_cart_text', $config->get( 'config_language_id' ) ); } else { echo 'Add to cart'; } ?></span></a></div>
					<?php } ?>
